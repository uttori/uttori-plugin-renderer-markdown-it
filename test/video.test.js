...
test('MarkdownItRenderer.render(content, config): can render a video', (t) => {
  ...
  t.is(MarkdownItRenderer.render(markdown, { ...MarkdownItRenderer.defaultConfig(), uttori: { ...MarkdownItRenderer.defaultConfig().uttori, allowedExternalDomains: [] } }), output);
});
...
test('MarkdownItRenderer.render(content, config): can render a video from an allowed domain', (t) => {
  ...
  t.is(MarkdownItRenderer.render(markdown, { ...MarkdownItRenderer.defaultConfig(), uttori: { ...MarkdownItRenderer.defaultConfig().uttori, allowedExternalDomains: ['wiki.superfamicom.org'] } }), output);
});
